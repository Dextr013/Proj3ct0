import { GameState, Hero, HeroClass, Race } from '../core/types';
import { createBaseHero } from '../game/heroes';
import { purchase } from '../core/platform';

function canAddHeroToMain(state: GameState): boolean {
...
function addHeroToMain(state: GameState, hero: Hero): void {
  const main = state.squads.find((s) => s.isMain);
  if (!main) return;
...
export function renderTavern(state: GameState, listEl: HTMLElement, onClose: () => void): void {
  listEl.innerHTML = '';
  const offers: Hero[] = [
