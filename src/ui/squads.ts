...
export function ensureSquadsInitialized(state: GameState): void {
...
export function startResourceLoop(state: GameState, onTick: () => void): number {
  const interval = window.setInterval(() => {
    if (state.settings.paused) return;
    state.squads.forEach((sq) => {
      if (!sq.isMain) {
        if (sq.task === 'Gathering') state.gold += 1;
        if (sq.task === 'Mining') state.crystals += 0.1;
      }
    });
    onTick();
  }, 1000);
